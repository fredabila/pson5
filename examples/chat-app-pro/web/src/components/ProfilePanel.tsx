...
function flattenLayer(layer: Record<string, unknown>): LayerEntry[] {
  ...
  for (const [domain, domainValue] of Object.entries(layer ?? {})) {
    ...
    if (Array.isArray(record.traits)) {
      for (const trait of record.traits as Array<Record<string, unknown>>) {
        out.push({
          domain,
          ...
          {entries.slice(0, 40).map((e, i) => (
            <li key={`${e.domain}/${e.key}/${i}`} className="lane__entry">
              ...
              <div className="lane__entry-value">
                <code>{e.value}</code>
                {e.confidence != null ? <span className="lane__confidence">{e.confidence.toFixed(2)}</span> : null}
              </div>
            </li>
          ))}
          {entries.length > 40 ? <li className="lane__entry lane__entry--overflow">+ {entries.length - 40} more</li> : null}
        </ul>
      )}
    </section>
  );
      ...
      <div className="profile profile--empty">
        <div className="profile__empty-message">
          ...
          <p>
            ...
            <strong> observed</strong>,<strong> inferred</strong>, and
            <strong> simulated</strong>. Everything persists to Postgres.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="profile">
      <header className="profile__head">
        <span className="profile__eyebrow">PROFILE · NEON POSTGRES</span>
        <h2 className="profile__title">What I know about you</h2>
      </header>
      <Lane index="01" label="OBSERVED" sublabel="facts you've told me directly" accent="observed" entries={observed} />
      <Lane index="02" label="INFERRED" sublabel="traits I've derived with confidence" accent="inferred" entries={inferred} />
      <Lane index="03" label="SIMULATED" sublabel="predictions I've generated in context" accent="simulated" entries={simulated} />
    </div>
  );
}
