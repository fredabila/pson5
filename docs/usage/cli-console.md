...
PSON5 now exposes two CLI surfaces:
...
- session and provider status
- a visible workflow guide
...
- when a question is staged, the prompt changes to `answer>`
- at that point you can type the answer directly without using `/answer`
...
- Tab completes slash commands and common argument values.
...
- `/home` brings the dashboard back if you get lost in other views.
- `/examples` shows a short tested walkthrough inside the main pane.
- `/provider-setup` writes local provider config under the current store root.
